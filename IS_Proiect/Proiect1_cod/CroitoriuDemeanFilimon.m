...
for g = 1 : m
    ...
    for i = 1 : n_id 
        for j = 1 : na+nb
            ...
            for q=0: p
                for r=0: q
                phi_id(k, n) = (X1id(i).^ (p - q)).* (X2id(j).^ (q-r).* (X3id(i)^r));
                if(q == p && p < g)
                    p = p + 1;
                    q = 0;
                else
...
subplot(2, 1, 2)
mesh(X1id, X2id, YY_id)
title('Aproximare date identificare')
xlabel('X1')
ylabel('X2')
zlabel('Y')
figure
...
minim_MSEid = 100000;
for i = 1 : m
    if vecMSE_id(i) < minim_MSEid
        minim_MSEid = vecMSE_id(i);
    end
end
